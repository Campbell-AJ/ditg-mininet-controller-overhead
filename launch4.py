...
from mininet.node import RemoteController, OVSSwitch, DefaultController, OVSController
...
def generate_elephant_flows(id, duration, net, log_dir):
    ...
    # create cmd
    cd_cmd = "cd ../D-ITG-2.8.1-r1023/bin "
    dst.cmdPrint(cd_cmd)

    recv_cmd = "./ITGRecv & "
    dst.cmdPrint(recv_cmd)

    src.cmdPrint(cd_cmd)
    
    send_cmd = "./ITGSend -T TCP -a "
    send_cmd+= dst.IP()
    send_cmd+= " -e 512 -t "
    send_cmd+= str(duration*1000)
    send_cmd+= " & "
    src.cmdPrint(send_cmd)
...
if __name__ == "__main__":
    ...
    if default_controller:
        ...
        net = Mininet(topo=topology, controller= None, host=CPULimitedHost, link=TCLink,
                      switch=OVSSwitch, autoSetMacs=True)
        net.addController("c1", controller = OVSController, ip=controller_ip, port=controller_port, protocols=["OpenFlow10"])
    else:
        net = Mininet(topo=topology, controller=None, host=CPULimitedHost, link=TCLink,
                      switch=OVSSwitch, autoSetMacs=True)
        net.addController('c1', controller=RemoteController, ip=controller_ip, port=controller_port, protocols=["OpenFlow10"])

    net.start()

    user_input = "QUIT"
    
    generate_flows(10, 0, 100, net, log_dir)
